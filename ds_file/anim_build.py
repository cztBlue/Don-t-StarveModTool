...
from ds_file.anim_util import *
from ds_file.anim_bank import AnimBank
from klei.symbol_map import GetSwapSymbol
from klei import atlas_image, optimize_image
...
class AnimBuild():
    ...
    def save_symbol_images(self, output_path, auto_completion=False):
        for symobl_name, frames in self.data["Symbol"].items():
            symobl_path = os.path.join(output_path, symobl_name)
            try_makedirs(symobl_path)
            for frame in frames:
                frame_name = f"{symobl_name}-{frame['framenum']}"
                self.symbol_images[frame_name].save(os.path.join(symobl_path, f"{symobl_name}-{frame['framenum']}.png"))
                if auto_completion:
                    for duration in range(1, frame["duration"]):
                        self.symbol_images[frame_name].save(os.path.join(symobl_path, f"{symobl_name}-{frame['framenum'] + duration}'(duration{frame['framenum']}').png"))
