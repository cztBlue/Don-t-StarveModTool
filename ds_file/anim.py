...
class DSAnim():
    ...
    def to_scml(self, output: str, mapping: bool=False, name: str=None):
        ...
        output = os.path.join(output, self.build.data["name"])
        try_makedirs(output)

        scml = Scml()
        scml_root = scml.getroot()
        self.build.to_scml(scml_root, self.bank, output, mapping)
        self.bank.to_scml(scml_root)
        scml.writr(os.path.join(output, self.build.data["name"] + ".scml"))
